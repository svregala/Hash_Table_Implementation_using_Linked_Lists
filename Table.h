...
class Table {
 ...
 private:
   ...
   - hashSize must be a non-negative number (though, a dynamic array of 0 size is considered useless)
   - data must be an array of pointers of type Node, and each node must be distinct in their keys from other nodes
   - hash table data must be of size hashSize
   */
   ...
   // private instance variable, a dynamic array
   ListType* data;

   // return number of non-empty buckets in table
   int findNonEmptyBuckets() const;

   // return longest chain in hash table 
